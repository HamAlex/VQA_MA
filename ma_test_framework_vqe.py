...
csv_filename = "vqe_masterarbeit_results.csv"
...
optimierer_dict = {
    ...
    'SPSA': SPSA(maxiter=int(max_iterations / 2))
}
...
with open(csv_filename, mode='a', newline='') as file:
    ...
    print(f"\nStarte Testreihen. Ergebnisse werden in {csv_filename} gesichert...")
    
    for mol_name, qubit_op in molecules:
        print(f"\n--- Analysiere {mol_name} (Benötigte Qubits: {qubit_op.num_qubits}) ---")
        ...
        print(f"-> Exakte (klassische) Referenz-Energie: {exact_energy:.6f} Hartree\n")
        
        for tiefe in ansatz_tiefen:
            for ansatz_art in ansatz_arten:
                for opt_name, optimizer in optimierer_dict.items():
                    for fehler_art in fehlerarten:
                        for p in fehlerraten:
                            ...
                            print(f"Berechne: {mol_name} | {ansatz_art} (T={tiefe}) | {opt_name} | {fehler_art if p > 0 else 'noiseless'} ({p*100:.1f}%) ...", end=" ", flush=True)
                            ...
                            try:
                                ...
                                writer.writerow([mol_name, ansatz_art, tiefe, opt_name, fehler_art if p > 0 else "noiseless", p, exact_energy, energie, result.cost_function_evals, dauer])
                                file.flush()
                                print(f"Abgeschlossen in {dauer}s (Energie: {energie:.4f})")
                                
                            except Exception as e:
                                print(f"Fehler bei der Berechnung: {str(e)}")
                                writer.writerow([mol_name, ansatz_art, tiefe, opt_name, fehler_art if p > 0 else "noiseless", p, exact_energy, "ERROR", 0, 0])
                                file.flush()
